...
# TODO: Replace <FILL IN> with appropriate code
pluralLengths = (pluralRDD.map(lambda word:len(word)-word.count(' '))
                 .collect())
print pluralLengths
...
def wordCount(wordListRDD):
    ...
    """
    wordCountTpls = (wordListRDD
                        .map(lambda word: (word, 1))
                        .reduceByKey(add))
    return wordCountTpls
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, key=lambda (word, knt): -knt)
