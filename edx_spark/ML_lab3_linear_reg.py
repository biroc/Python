...
def parsePoint(line):
    ...
    line = line.split(",");
    l = len(line);
    return LabeledPoint(line[0],line[1:l]);
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(lambda ln: parsePoint(ln));
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda tp: squaredError(tp[0], tp[1])).sum()/labelsAndPreds.count());
...
# TODO: Replace <FILL IN> with appropriate code
meanTrain = parsedTrainData.map(lambda lp: lp.label).mean();
labelsAndPredsTrain = parsedTrainData.map(lambda lp: (lp.label, meanTrain));
rmseTrainBase = calcRMSE(labelsAndPredsTrain);

labelsAndPredsVal = parsedValData.map(lambda lp: (lp.label, meanTrain));
rmseValBase = calcRMSE(labelsAndPredsVal);

labelsAndPredsTest = parsedTestData.map(lambda lp: (lp.label, meanTrain));
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(label, new_features);
